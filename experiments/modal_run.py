"""Run the copy-mixture record attempt on a single Modal H100, on upstream record #91 (1250 + 40 steps).

This is the launcher from the masking experiment on #89, moved to the #91 code base. Runs from here
live under logs91/ and dumps91/ on the Volume, so they can never be pooled with, or skipped because
of, a #89 run with the same tag. The spend ledger (logs/_spend) is shared: the budget is one budget.
Only --arm baseline exists here. The sections below that mention masking describe the #89 harness.

Evidence for the copy-mixture record (the mixture is on by default; its loss is the run's val_loss):
...
VOL = "/vol"
VOL_RUNS, VOL_DUMPS = "logs91", "dumps91"  # per code base; the spend ledger stays under logs/_spend
...
def run_copy_offline(tag: str):
    ...
    exec(re.search(r"^# Document-local copy mixture.*?(?=^# -{20,}\n# Training Management)",
                   open("/repo/train_gpt.py").read(), re.M | re.S).group(0), ns)
    dump = torch.load(f"{VOL}/{VOL_DUMPS}/{tag}/rank0.pt")
    chunk = 4 * 64 * 1024 * 8 // 8
...
def _fetch(logs: Path) -> int:
    ...
    n = 0
    for root, spend_only in ((VOL_RUNS, False), ("logs", True)):  # this code base's runs, everyone's spend
        try:
            entries = vol.listdir(root, recursive=True)
        except Exception:
            continue  # nothing has been logged there yet
        for e in entries:
            dest = logs / Path(e.path).relative_to(root)
            is_spend = dest.parent.name == "_spend"
            wanted = is_spend if spend_only else (dest.name in KEEP and not is_spend)
...
def _tag(arm: str, p: str, p_end: str, seed: str, steps: str, rep: str = "1", copy_mix: bool = True, ext: str = "") -> str:
